...
def pose_record(pose):
    ...
    if ((int(new_ut) - int(ut)) % 5 == 0):
        s = str(new_ut) + ':' + ' (' + str(math.floor(nose.yx[0])) + ',' + str(math.floor(nose.yx[1])) + ')  ' \
        ...
        with open(path_record,mode='r') as f:
            ...
            last_line = lines[-1]
            last_time = re.findall('^[0-9]+', last_line)
            #print(last_time)
        
        if(str(new_ut) not in last_time):
