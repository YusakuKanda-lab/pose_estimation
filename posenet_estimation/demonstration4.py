...
df = pd.DataFrame(data,
                  columns=['Datetime','nose_x','nose_y','left_eye_x','left_eye_y','right_eye_x','right_eye_y', \
...
def pose_record(pose):
    ...
    if(last_line != new_dt_str[17:19]):
        if ((int(new_ut) - int(ut)) % 5 == 0):
            ...
            data_list = [
                    new_dt_str, \
                    str(nose.yx[0]), str(nose.yx[1]),\
                    str(left_eye.yx[0]), str(left_eye.yx[1]), str(right_eye.yx[0]), str(right_eye.yx[1]), \
                    str(left_ear.yx[0]), str(left_ear.yx[1]), str(right_ear.yx[0]), str(right_ear.yx[1]), \
                    str(left_shoulder.yx[0]), str(left_shoulder.yx[1]), str(right_shoulder.yx[0]), str(right_shoulder.yx[1]), \
                    str(left_elbow.yx[0]), str(left_elbow.yx[1]), str(right_elbow.yx[0]), str(right_elbow.yx[1]), \
                    str(left_wrist.yx[0]), str(left_wrist.yx[1]), str(right_wrist.yx[0]), str(right_wrist.yx[1]), \
                    str(left_hip.yx[0]), str(left_hip.yx[1]), str(right_hip.yx[0]), str(right_hip.yx[1]), \
                    str(left_knee.yx[0]), str(left_knee.yx[1]), str(right_knee.yx[0]), str(right_knee.yx[1]), \
                    str(left_ankle.yx[0]), str(left_ankle.yx[1]), str(right_ankle.yx[0]), str(right_ankle.yx[1])]
            df.loc[len(df)]=data_list
            df.to_csv("record/pose_data.csv")
            print(df)
